...
function renderKnownContent(content = {}) {
  const lead = [];
  const details = [];
  const hiddenKeys = new Set(['options', 'categories', 'items', 'itemIds', 'claims', 'availableTokens', 'auditoryOptions', 'relationOptions']);
...
function renderStimulus(stimulus, index) {
  ...
  if (stimulus.type === 'CONTROLLED_AUDIO') {
    return `<div class="media-placeholder" role="status"><strong>Áudio controlado pendente</strong><span>${esc(stimulus.mediaId || id)}</span><small>Esta atividade pode ser implementada, mas o estímulo final ainda precisa ser ligado ao mediaId.</small></div>`;
  }
...
function renderStimuli(activity) {
  const stimuli = activity?.stimuli || [];
...
function classifyItems(block) {
  ...
  if (Array.isArray(content.items) && content.items.length) return content.items.map((item, index) => ({ key: item.id || String(index), label: item.stimulus ?? item.text ?? item.claim ?? item.spokenContent ?? `Item ${index + 1}` }));
  ...
  const answerItems = block.activity?.evaluation?.answerKey?.items || block.activity?.evaluation?.answerKey || {};
  return Object.keys(answerItems).filter(key => !['correct', 'correctIndex', 'correctSequence', 'auditoryCorrect', 'relationCorrectIndex'].includes(key)).map((key, index) => ({ key, label: `Item ${index + 1}` }));
}

function classifyCategories(block) {
  const content = block.content || {};
  if (Array.isArray(content.categories)) return content.categories;
  const key = block.activity?.evaluation?.answerKey?.items || block.activity?.evaluation?.answerKey || {};
...
function renderClassify(block) {
  ...
  if (!items.length || !categories.length) return renderOpenInput(block, 'Resposta da classificação');
  return `<div class="classify-grid">${items.map(item => `
    <label class="classify-row"><span>${valueText(item.label)}</span>
      <select name="classify:${esc(item.key)}" required>
...
function renderSequence(block) {
  ...
  return `<div class="sequence-builder" data-sequence-builder>
    <div class="token-bank" aria-label="Itens disponíveis">${tokens.map((token, index) => `<button type="button" class="token-button" data-sequence-token="${index}" data-value="${esc(typeof token === 'string' ? token : JSON.stringify(token))}">${valueText(token)}</button>`).join('')}</div>
...
function renderInteraction(block) {
  ...
  switch (block.activity?.interaction) {
    ...
    default: return `<div class="unsupported-state" role="alert"><strong>Interação ainda não suportada</strong><span>${esc(block.activity?.interaction || 'desconhecida')}</span></div>`;
  }
}
...
function evaluateDeterministic(form, block) {
  ...
  if (interaction === 'CLASSIFY' || interaction === 'MATCH') {
    const expected = key.items || key;
    ...
    return { complete: true, correct: hits / entries.length >= threshold, score: hits / entries.length };
  }

  if (interaction === 'SEQUENCE' || interaction === 'ORDER') {
    const value = JSON.parse(form.querySelector('input[name="sequence"]')?.value || '[]');
  ...
  if (interaction === 'COMPOSITE' && Object.prototype.hasOwnProperty.call(key, 'auditoryCorrect')) {
    ...
    const auditoryValue = block.content?.auditoryOptions?.[Number(auditory.value)];
    return { complete: true, correct: normalizeComparable(auditoryValue) === normalizeComparable(key.auditoryCorrect) && Number(relation.value) === key.relationCorrectIndex };
  }
...
function bindSequence(builder) {
  const input = builder.querySelector('input[name="sequence"]');
  const answer = builder.querySelector('[data-sequence-answer]');
  let sequence = [];
  const sync = () => {
    input.value = JSON.stringify(sequence);
    answer.innerHTML = sequence.length ? sequence.map(item => `<span class="token">${esc(item)}</span>`).join(' ') : 'Sua sequência aparecerá aqui.';
  };
